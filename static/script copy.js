class ChatApp {
    ...
    async streamResponse(typingIndicator, chat) {
        ...
        // Заменяем typing indicator на пустое сообщение ассистента
        typingIndicator.remove();
        ...
        try {
            ...
            // Сохраняем готовое сообщение в историю
            if (fullContent) {
            ...
            if (error.name === 'AbortError') {
                // Остановлено пользователем — сохраняем что успело накопиться
                if (fullContent) {
                    chat.messages.push({ role: 'assistant', content: fullContent });
                    chat.messageCount++;
                }
